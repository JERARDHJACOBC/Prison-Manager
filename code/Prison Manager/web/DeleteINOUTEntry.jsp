...
<FORM Name='DelForm' METHOD=POST ACTION="">
		...
		<TABLE align=center width="25%" bordercolor=#D8D8D8 border=1>
                <TR>
		<TH class='row_title' colspan=2>Delete IN-OUT Entry</TH>
		</TR>
		<TR>
...
<%
	try{
			...
			if(Condition.trim().equalsIgnoreCase("undefined")){
				...
				Query1 = "Select * from inoutregister";
			}
			else{
				System.out.println("in else");
				Query1 = "Select "+Condition+" from inoutregister group by "+Condition;
			}
			String str="";
...
<%
	...
	ResultSet rs=null;
	Statement stmt=null;
	String Value = request.getParameter("inoutid");
	try{
			con = com.PrisonManager.ConnectionPool.getConnection();
			stmt =  con.createStatement();
	
			
			String Query = "Delete from inoutregister where inoutid='"+Value+"'";
