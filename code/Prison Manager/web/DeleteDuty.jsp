...
<FORM Name='DelForm' METHOD=POST ACTION="">
		...
		<TR>
		<TH class='row_title' colspan=2>Delete Duty Entry</TH>
		</TR>
                 <TR>
...
<%
	try{
			...
			if(Condition.trim().equalsIgnoreCase("undefined")){
				...
				Query1 = "Select * from DutyRegister";
			}
			else{
				System.out.println("in else");
				Query1 = "Select "+Condition+" from DutyRegister group by "+Condition;
			}
			String str="";
...
<%
	...
	ResultSet rs=null;
	Statement stmt=null;
	String Value = request.getParameter("DutyId");
	try{
			con = com.PrisonManager.ConnectionPool.getConnection();
			stmt =  con.createStatement();
	
			
			String Query = "Delete from DutyRegister where DutyId='"+Value+"'";
