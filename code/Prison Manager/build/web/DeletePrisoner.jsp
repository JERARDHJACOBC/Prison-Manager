...
if(!(request.getMethod().equals("POST")))
{
	...
	ResultSet rs=null;
	Statement stmt=null;
	String Condition = "PrisonerId";
	String[] ConValues = new String[2000];
	int ConCount=0,i=0;
	String Query1="";
%>
<FORM Name='DelForm' METHOD=POST ACTION="DeletePrisoner.jsp">
		<TABLE align=center width="25%" bordercolor=#D8D8D8 border=1>
		<TR>
		<TH class='row_title' colspan=2>Delete Prisoner</TH>
		</TR>
                 <TR>
...
<%
	try{
			...
			if(Condition.trim().equalsIgnoreCase("undefined")){
				...
				Query1 = "Select * from prisoners";
			}
			else{
				System.out.println("in else");
				Query1 = "Select "+Condition+" from prisoners group by "+Condition;
			}
			String str="";
...
<%
	...
	ResultSet rs=null;
	Statement stmt=null;
	String Value = request.getParameter("PrisonerId");
	try{
			con = com.PrisonManager.ConnectionPool.getConnection();
			stmt =  con.createStatement();
	
			
			String Query = "Delete from prisoners where PrisonerId='"+Value+"'";
			int result = stmt.executeUpdate(Query);
			if( result > 0)	{
                            Query = "Delete from casedetails where PrisonerId='"+Value+"'";
                            result = stmt.executeUpdate(Query);
                            System.out.println("Case details deleted successfully--->"+result);
